...
/**
 * Перед каждым тестовым методом создаём новое дерево методом fillTree().
 * Тестируем подмножества. Методы этого класса для всех трёх типов подмножеств делятся на 2 типа:
 * тестирование на рамзер и содержание множества и Relation-тесты на связность. В первом случае всё понятно интуитивно.
 * Во втором случае поочерёдно проделываем 4 вида операций: добавляем в дерево, проверяем добавилось ли в мн-во
 * (если соответствует границам), далее проделываем то же самое с удалением, далее проделываем эти две операции,
 * но вызывая их уже из подмножества, и проверяем, был ли эффект на дереве.
 */
class SubSetTest {
